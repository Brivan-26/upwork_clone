...
@section('content')
  <h1 class="text-3xl text-green-500">Board table</h1>
  ...
  <h2 class="text-xl my-2"><svg class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    </svg> Your favorates jobs ({{ auth()->user()->likes()->count() }})</h2>
  ...
  <section class="text-sm text-gray-700 w-full">
    ...
    Your proposals ({{ auth()->user()->jobs()->count() }})</h2>
    ...
    <div class="mb-3">
      ...
      <li class="mt-2">• "{{ $proposal->coverLetter->content }}" From
      <strong>
      {{ $proposal->user->name }}
      </strong>
      </li>
      @if ($proposal->is_validated)
      <span class="bg-white border border-green-500 text-xs p-1 my-2 inline-block text-green-500 rounded">Already validated</span>
      @else
      <a href="#" class="bg-green-500 text-xs py-2 px-2 mt-2 mb-3 inline-block text-white hover:bg-green-200 hover:text-green-500 duration-200 transition rounded">Confirm proposal</a>
      @endif
      @endforeach
      </ul>
    </div>
